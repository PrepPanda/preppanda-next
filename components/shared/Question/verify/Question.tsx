...
const Question = ({ questiondata, uniqueValue, deleteQuestion, handleAnswerChange=() => {}, disabled = false }: any) => {
    ...
    return (
        <div className="relative border-gray-300 flex flex-col rounded-md p-4 px-10 border-2 m-4 basis-1/4">
            {!disabled &&
                <RxCrossCircled onClick={() => deleteQuestion(uniqueValue)} className="absolute -top-3 -right-3 text-red-500 text-2xl" />
            }
            {editedQuestion}
            <hr className="my-4" />
